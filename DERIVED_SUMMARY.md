...
- **Two invariants and a sign** (corrected): ~~baryon number = winding charge ΣW~~ (**further corrected 18 Aug 2026, CHRG0: W is electric charge, not baryon number — see the winding row below; baryon number has no topological carrier**); magnetic helicity = flux linking (2π)²ΣWᵢWⱼLkᵢⱼ (since A=∇θ ⇒ winding lines are flux tubes, Φ=2πW); chirality = sign of the helicity. **The third genuinely is a face of the second. The first is independent of both.**
...
| winding W ∈ ℤ (π₁(S¹)) | **electric charge** (signed) — and this is now *forced*, not chosen. ⚠ **Corrected 18 Aug 2026 (BMCA0 → CHRG0):** this row previously also equated W with **baryon number**. It cannot be both — the neutron needs B=1 and Q=0 at once. **CHRG0 resolved it: the electron is a spin-½ fermion, which by this repo's own SL = W relation requires odd W, while its baryon number is 0. So W is electric charge, and baryon number has NO topological carrier** (helicity is quadratic and vanishes at W=0; the Noether charge is continuous). Consequence: the proton-stability prediction is withdrawn (`PREDICTIONS.md`). **Still open: the Coulomb/EM sector derives charge as the *Noether* charge j⁰=ρ²ω (§ above), while charge quantisation uses W — two different objects under one name. Unresolved.** |
